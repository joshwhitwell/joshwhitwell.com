...
<body>
  ...
  @foreach ($programDayLog->workoutProgramDayExerciseLogs as $exerciseLog)
    ...
    @foreach ($exerciseLog->workoutProgramDayExerciseSetLogs as $setLog)
      <h3>{{ $setLog->workoutProgramDayExerciseSet->is_warm_up ? 'Warm Up' : 'Set' }} {{ $setLog->workoutProgramDayExerciseSet->order }} @if ($setLog->workoutProgramDayExerciseSet->is_optional) <small>(Optional)</small>@endif</h3>

      <div>
        <form action="{{ route('set-logs.store', $setLog) }}" method="POST">
          @csrf
          <label for="reps">
            Reps
